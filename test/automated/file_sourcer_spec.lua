...
describe('file_sourcer', function()
  ...
  after_each(function()
    vim.fn.delete('test/fixtures/file_sourcer', 'rf')
  end)
  ...
  it('should return an error if the file is not readable', function()
    ...
    assert.are.same('File not readable: test/fixtures/file_sourcer/non_existent.lua', result.error.message)
  end)
  ...
  it('should return an error if the file has a syntax error', function()
    ...
    assert.is_true(result:is_err())
    assert.truthy(result.error.message:find('Error sourcing test/fixtures/file_sourcer/syntax_error.lua'))
  end)
  ...
  it('should source all lua files in a directory', function()
    ...
    local result = file_sourcer.source_directory('test/fixtures/file_sourcer', { recursive = false })
    assert.is_true(result:is_ok())
  ...
  it('should return an error if any of the files have a syntax error', function()
    ...
    local result = file_sourcer.source_directory('test/fixtures/file_sourcer', { recursive = false })
    assert.is_true(result:is_err())
    local err_result = result:unwrap_err()
    assert.are.same(1, err_result.files_sourced)
    assert.are.same(1, #err_result.errors)
  end)
end)
