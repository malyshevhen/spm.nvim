...
local function setup(config, force_reinstall)
  ...
  if force_reinstall or is_stale then
    ...
    local parse_success, result = pcall(parse_config, config.plugins_toml_path)
    if not parse_success and type(result) == 'string' then
      return false, result
    elseif not parse_success then
      local msg = string.format('Failed to parse plugins.toml: %s', result or 'unknown error')
      logger.error(msg, 'PluginManager')
  ...
  local install_success, install_error = pack_installer.install(flattened_plugins)
